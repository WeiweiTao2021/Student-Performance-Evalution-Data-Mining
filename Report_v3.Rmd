...
```{r,echo=FALSE,include=TRUE,message = FALSE, warning = FALSE, out.width='\\textwidth', fig.width=5,fig.height=4, fig.align='center', fig.cap = "Distribution of G1 grade for students whose G2 equals 0"}
...
**Hypothesis 1:** $$H_0: \mu_{G3} = \mu_{G1}$$  $$H_a: \mu_{G3} \neq \mu_{G1}$$

**Hypothesis 2:** $$H_0: \mu_{G3} = \mu_{G2}$$  $$H_a: \mu_{G3} \neq \mu_{G2}$$
...
Paired data correlation is a way to understand the relationship between multiple variables and attributes. Using correlation, we can obtain insights whether one or multiple attributes are associated with each other. Figure 6 shows the correlation plot with correlation coefficients for significantly correlated variables. The correlation coefficients shown are calculated using Spearman's rank-order correlation method since most of the continuous variables are ordinal.
...
names(df_num)[names(df_num) == 'age'] <- "Age"
names(df_num)[names(df_num) == 'goout'] <- "Goout"
names(df_num)[names(df_num) == 'health'] <- "Health"
names(df_num)[names(df_num) == 'absences'] <- "Absences"
names(df_num)[names(df_num) == 'freetime'] <- "Freetime"
names(df_num)[names(df_num) == 'Workday alcohol'] <- "Workday Alcohol"
names(df_num)[names(df_num) == 'Weekend alcohol'] <- "Weekend Alcohol"
...
#final model
final <- lm(I(G3) ~ (sex + age +  `Mother Education` +
                       `Mother Job` + `Father Job` + studytime
                     + `Past Failures` + schoolsup + health + goout), data=df_lr)
summary(final)
```
...
```{r,echo=FALSE,include=TRUE,message = FALSE, warning = FALSE, out.width='\\textwidth', fig.width=5,fig.height=6, fig.align='center', fig.cap="Accuracy of the model based on the number of features included and feature importance."}
...
Lastly, based on Logistic regression modeling, whether a student able to improve their score from G1 to final exam is highly correlated the number of adscense from class. Thus, in order to improve a student's grade, we would suggest him/her to attend as many classes as possible.
## References
...
[3] \url{https://www.npr.org/2012/04/09/150062919/lack-of-graduates-hampers-portugals-recovery}

[4] \url{https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html}

[5] \url{https://www.gerkovink.com/miceVignettes/Convergence_pooling/Convergence_and_pooling.html}

[6] \url{https://towardsdatascience.com/effective-feature-selection-recursive-feature-elimination-using-r-148ff998e4f7}

[7] \url{https://stats.stackexchange.com/questions/2492/is-normality-testing-essentially-useless}
